...
async function magicLink({ query }: Request, res: Response): Promise<unknown> {
  ...
  const useCookie =
    typeof query.cookie !== "undefined" ? query.cookie === "true" : true
  ...
  if (action === "sign-up") {
    ...
    if (!affected_rows) {
      ...
      throw res.boom.unauthorized("Invalid or expired token.")
    }
  ...
  if (action === "log-in") {
    if (APPLICATION.REDIRECT_URL_SUCCESS) {
      return res.redirect(
        `${APPLICATION.REDIRECT_URL_SUCCESS}?refresh_token=${refresh_token}`
      )
    }
    ...
    res
      .status(200)
      .send(
        "Your account has been activated. You can close this window and login"
      )
  } else {
    res.status(400)
  }
